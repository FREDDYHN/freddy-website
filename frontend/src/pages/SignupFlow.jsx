...
const MATERIALS = [
  { key: 'glass', label: '玻璃 / Glas' },
  { key: 'paper', label: '纸/纸板 / Papier/Pappe' },
  { key: 'ferrous', label: '黑色金属 / Eisenmetalle' },
  { key: 'aluminium', label: '铝 / Aluminium' },
  { key: 'plastics', label: '塑料 / Kunststoffe' },
  { key: 'composites', label: '复合材料 / Verbunde' },
  { key: 'cartons', label: '饮料纸盒 / Getränkekarton' },
  { key: 'other', label: '其他 / Sonstige' },
]

const TIERS = [
  { key: 'basic', name: '基础 Basic', price: 89, color: 'border-gray-200', desc: '核心AR服务：系统参与、LUCID数据申报、官方通信' },
  { key: 'standard', name: '标准 Standard', price: 159, color: 'border-primary', desc: '基础+优先响应(48h)+扩展分类咨询+年度合规简报', featured: true },
  { key: 'premium', name: '高级 Premium', price: 249, color: 'border-gray-200', desc: '标准+完整性声明协调+专属客户经理+24h响应' },
]
...
export default function SignupFlow() {
  ...
  const addItem = () => {
    if (!newItem.kg || parseFloat(newItem.kg) <= 0) return
    setForm(f => ({
      ...f,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body = {
        ...
        packaging_items: form.packaging_items.map(p => ({ material_type: p.material, category: p.category, estimated_kg: p.kg })),
        tier: form.tier,
      }
      ...
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 text-left space-y-3 mb-8">
          ...
          {result.payment?.payUrl && (
            <a href={result.payment.payUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-4 px-6 py-3 bg-green-600 text-white rounded-lg font-semibold">
              去支付 →
            </a>
          )}
        </div>
        <p className="text-sm text-gray-500">支付完成后合同即激活。我们将向 {form.contact_email} 发送合同PDF和LUCID注册指南。</p>
      </div>
    )
  }
    ...
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      <div className="flex justify-center mb-10">
        {STEPS.map((s, i) => (
          <div key={i} className="flex items-center">
            ...
            {i < 3 && <div className="w-8 md:w-16 h-0.5 mx-2 bg-gray-200"><div className={`h-full ${i < step ? 'bg-primary' : ''}`} /></div>}
          </div>
        ))}
      </div>
      ...
      {step === 0 && (
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium mb-1">公司名称 *</label><input value={form.company_name} onChange={e => update('company_name', e.target.value)} className="w-full border rounded-lg p-2 text-sm" placeholder="您的公司全称" /></div>
            <div><label className="block text-sm font-medium mb-1">联系人 *</label><input value={form.contact_name} onChange={e => update('contact_name', e.target.value)} className="w-full border rounded-lg p-2 text-sm" placeholder="姓名" /></div>
            <div><label className="block text-sm font-medium mb-1">电子邮箱 *</label><input type="email" value={form.contact_email} onChange={e => update('contact_email', e.target.value)} className="w-full border rounded-lg p-2 text-sm" placeholder="用于接收合同PDF" /></div>
            <div><label className="block text-sm font-medium mb-1">手机号</label><input value={form.contact_phone} onChange={e => update('contact_phone', e.target.value)} className="w-full border rounded-lg p-2 text-sm" /></div>
            <div><label className="block text-sm font-medium mb-1">微信号</label><input value={form.wechat_id} onChange={e => update('wechat_id', e.target.value)} className="w-full border rounded-lg p-2 text-sm" placeholder="选填" /></div>
          </div>
          <div className="flex justify-end pt-4">
            <button onClick={() => setStep(1)} disabled={!form.company_name || !form.contact_name || !form.contact_email}
      ...
      {step === 1 && (
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 space-y-4">
          ...
          <p className="text-sm text-gray-500">请添加您在德国市场使用的所有包装类型。不知道重量？可以用我们的估算器。</p>
          <div className="flex flex-wrap gap-2">
            <select value={newItem.material} onChange={e => setNewItem(n => ({ ...n, material: e.target.value }))} className="border rounded-lg p-2 text-sm flex-1 min-w-[200px]">
              {MATERIALS.map(m => <option key={m.key} value={m.key}>{m.label}</option>)}
            </select>
            <select value={newItem.category} onChange={e => setNewItem(n => ({ ...n, category: e.target.value }))} className="border rounded-lg p-2 text-sm w-24">
              <option value="B2C">B2C</option><option value="B2B">B2B</option>
            </select>
            <input type="number" value={newItem.kg} onChange={e => setNewItem(n => ({ ...n, kg: e.target.value }))} placeholder="公斤/年" className="border rounded-lg p-2 text-sm w-28" />
      ...
      {step === 3 && (
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 space-y-4">
          ...
          <div>
            ...
            <input value={form.signer_name} onChange={e => update('signer_name', e.target.value)} className="w-full border rounded-lg p-2 text-sm" placeholder="请输入您的姓名作为电子签名" />
          </div>
          <div className="flex justify-between pt-4">
            <button onClick={() => setStep(2)} className="px-4 py-2 border rounded-lg text-sm">← 上一步</button>
            <button onClick={handleSubmit} disabled={!form.agreed || !form.signer_name || submitting}
