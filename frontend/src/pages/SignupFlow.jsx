...
export default function SignupFlow() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // 3. Store token, contract info, and redirect to Dashboard
      sessionStorage.setItem('token', loginData.token)
      sessionStorage.setItem('user', JSON.stringify(loginData.user))
      // Contract was already generated by backend — pass download URL to Dashboard
      if (data.download_url) sessionStorage.setItem('pending_download', data.download_url)
      if (data.contract_number) sessionStorage.setItem('pending_contract_no', data.contract_number)
      navigate('/dashboard')
