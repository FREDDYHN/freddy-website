...
export default function SignupFlow() {
  ...
  const validate = (s) => {
    ...
    if (s === 0) {
      ...
      if (!form.registered_address_en.trim()) e.registered_address_en = '请输入注册地址（英文）'
      ...
      if (form.registered_address_en.trim() && containsChinese(form.registered_address_en)) e.registered_address_en = '请输入英文或拼音（不能包含中文）'
  ...
  return (
    <div className="min-h-screen py-10" style={{background:'#f4f2ef'}}><div className="max-w-4xl mx-auto px-4">
      ...
      {/* Submitted state — show after successful contract creation */}
      {submitted && (
        <div className="bg-white border border-gray-100 rounded-lg p-10 text-center space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[['company', '公司'], ['individual', '个人']].map(([k, label]) => (
                <button key={k} type="button" onClick={() => update('entity_type', k)}
                  className={`flex-1 py-2 rounded-md text-sm font-medium border transition-colors ${form.entity_type === k ? 'border-primary bg-primary/5 text-primary' : 'border-gray-200 text-gray-500 hover:border-gray-300'}`}>
                  {label}
                </button>
              ))}
            </div>
      ...
      {step === 1 && isPkg && (
        <div className="bg-white border border-gray-100 rounded-lg p-6 space-y-4">
          ...
          <ul className="text-sm text-gray-500 list-disc pl-5 space-y-1">
            <li>2026年度<span className="font-semibold text-orange-600">已经自行做过预申报的</span>，需将已预申报的包装种类和重量，如实填写在下方，并在登录客户端后<span className="font-semibold text-orange-600">上传预申报凭证/发票</span>。您无需缴纳预申报费。</li>
            <li>2026年度<span className="font-semibold text-orange-600">还没有预申报的</span>，请在下方填写预申报的包装种类和重量，系统将为您算出金额，在您登录客户端后，请<span className="font-semibold text-orange-600">查看“费用明细”并完成付款，上传付款凭证</span>。</li>
          </ul>
          {/* 8 种材料平铺表格 */}
