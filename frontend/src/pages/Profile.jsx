...
export default function Profile() {
  ...
  if (loading) return <div className="max-w-2xl mx-auto px-4 py-16"><div className="animate-pulse h-32 bg-border/15 rounded" /></div>
  if (!userStr) return <div className="max-w-2xl mx-auto px-4 py-16 text-center"><p className="text-text2">请先 <Link to="/login" className="text-cyan underline">登录</Link></p></div>
  if (isAdmin) return <div className="max-w-2xl mx-auto px-4 py-16 text-center"><p className="text-text2">管理员账户无需维护公司资料。请前往 <Link to="/admin" className="text-cyan underline">管理后台</Link></p></div>
  if (!profile) return <div className="max-w-2xl mx-auto px-4 py-16 text-center"><p className="text-text2">未找到客户资料，请联系客服。</p></div>

  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      <h1 className="text-2xl font-bold mb-2">账户管理</h1>
      <p className="text-text2 text-sm mb-8">{profile.company_name}</p>

      {/* Profile Form */}
      <form onSubmit={handleSave} className="bg-card rounded-xl p-6 shadow-sm border border-card space-y-4 mb-8">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-text2 mb-1">公司英文名</label>
            <input value={profile.company_name_en || ''} onChange={e => setProfile(p => ({...p, company_name_en: e.target.value}))}
              className="w-full border rounded-lg p-2.5 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-text2 mb-1">统一社会信用代码</label>
            <input value={profile.uscc || ''} onChange={e => setProfile(p => ({...p, uscc: e.target.value}))}
              className="w-full border rounded-lg p-2.5 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-text2 mb-1">法定代表人</label>
          ...
          <div>
            <label className="block text-sm font-medium text-text2 mb-1">手机号</label>
            <input value={profile.contact_phone || ''} onChange={e => setProfile(p => ({...p, contact_phone: e.target.value}))}
              className="w-full border rounded-lg p-2.5 text-sm" />
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-text2 mb-1">注册地址</label>
            <input value={profile.registered_address || ''} onChange={e => setProfile(p => ({...p, registered_address: e.target.value}))}
              className="w-full border rounded-lg p-2.5 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-text2 mb-1">LUCID注册号</label>
          ...
          <div>
            <label className="block text-sm font-medium text-text2 mb-1">微信</label>
      ...
      {/* Password Change */}
      <form onSubmit={handlePassword} className="bg-card rounded-xl p-6 shadow-sm border border-card space-y-4">
        <h2 className="font-bold text-lg">修改密码</h2>
        <div className="grid md:grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-text2 mb-1">当前密码</label>
            <input type="password" value={pw.current} onChange={e => setPw(p => ({...p, current: e.target.value}))}
              className="w-full border rounded-lg p-2.5 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-text2 mb-1">新密码</label>
            <input type="password" value={pw.newPw} onChange={e => setPw(p => ({...p, newPw: e.target.value}))}
              className="w-full border rounded-lg p-2.5 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-text2 mb-1">确认新密码</label>
        ...
        <button type="submit" className="px-6 py-2 bg-cyan text-white rounded-lg font-medium">
          修改密码
        </button>
      </form>
    </div>
