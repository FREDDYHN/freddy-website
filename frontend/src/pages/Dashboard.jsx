...
export default function Dashboard() {
  ...
  if (error === 'login_required') {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        ...
        <Link to="/login" className="px-6 py-2 bg-primary text-white rounded-lg font-medium">前往登录 →</Link>
      </div>
    )
  }
  ...
  if (error) return <div className="max-w-4xl mx-auto px-4 py-16 text-center"><div className="text-4xl mb-4">⚠️</div><h1 className="text-xl font-bold mb-2">加载失败</h1><p className="text-gray-500 mb-4">{error}</p><button onClick={() => window.location.reload()} className="px-4 py-2 border rounded-lg text-sm">重试</button></div>
  if (!data?.contracts?.length) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        <h1 className="text-2xl font-bold mb-4">暂无合同</h1>
        <p className="text-gray-500 mb-6">请先 <Link to="/signup" className="text-primary underline">在线签约</Link></p>
      </div>
    )
  }
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="flex flex-wrap items-center justify-between mb-8">
        ...
        <div className="flex gap-2">
          <Link to="/profile" className="px-3 py-1 text-sm border rounded-full text-gray-600 hover:bg-gray-50">✏️ 编辑资料</Link>
      ...
      <div className={`rounded-xl p-6 mb-8 shadow-sm border ${c.lucid_confirmed ? 'bg-green-50 border-green-200' : 'bg-amber-50 border-amber-200'}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">{c.lucid_confirmed ? '合规已生效。' : '请在LUCID系统中将 LIVANTO GmbH 指定为授权代表。'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-8">
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          ...
          <label className={`px-4 py-2 border rounded-lg text-sm font-medium cursor-pointer hover:bg-gray-50 ${uploading ? 'opacity-50' : ''}`}>
        ...
        {uploads.length > 0 && (
          <div className="border-t pt-4">
            ...
            <div className="space-y-2">
              {uploads.map(f => (
                <div key={f.stored_path} className="flex justify-between items-center text-sm bg-gray-50 rounded-lg p-3">
                  <span>{f.original_name}</span>
                  <span className="text-xs text-gray-400">{new Date(f.uploaded_at).toLocaleDateString('zh-CN')}</span>
                  <a href={`/api/uploads/${f.id || '0'}/download`} className="text-primary text-xs font-medium">下载</a>
                </div>
              ))}
            </div>
          </div>
      ...
      {c.status === 'pending_payment' && bankInfo && (
        <div className="bg-blue-50 rounded-xl p-6 shadow-sm border border-blue-200 mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-3 text-sm">
            <div><span className="text-gray-500">银行：</span>{bankInfo.bank_name}</div>
            <div><span className="text-gray-500">账户名：</span>{bankInfo.account_name}</div>
            <div><span className="text-gray-500">账号：</span>{bankInfo.account_number}</div>
            <div><span className="text-gray-500">SWIFT：</span>{bankInfo.swift}</div>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-8">
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          {data.packaging.length === 0 ? <p className="text-sm text-gray-400">暂无包装数据</p> :
            data.packaging.map(p => (
              <div key={p.id} className="flex justify-between items-center text-sm py-2 border-b border-gray-50">
                <span>{p.material_type}</span><span className="text-gray-400 text-xs">{p.packaging_category}</span>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          {data.payments.length === 0 ? <p className="text-sm text-gray-400">暂无支付记录</p> :
            data.payments.map(p => (
              <div key={p.id} className="flex justify-between items-center text-sm py-2 border-b border-gray-50">
                <span>¥{p.amount_cny} <span className="text-gray-400 text-xs">{p.payment_method === 'wechat' ? '微信' : p.payment_method === 'alipay' ? '支付宝' : p.payment_method}</span></span>
