  ...
  return (
    <div className="bg-white border border-gray-100 rounded-lg">
      ...
      >
        ...
        {/* Mini progress in collapsed header */}
        ...
        <span className={`text-[11px] font-medium flex-shrink-0 ${
          allDone ? 'text-green-500' : 'text-gray-400'
        }`}>
          {allDone ? '已完成' : `${doneCount}/${steps.length}`}
        </span>
        <span className={`text-gray-300 text-xs flex-shrink-0 transition-transform duration-300 ${
              ...
              return (
                <div key={step.key} className="flex flex-col items-center relative z-10" style={{ width: `${100 / steps.length}%`, minWidth: 0 }}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-base transition-all duration-300 ${
                    ...
                    active ? 'bg-primary text-white ring-4 ring-primary/15 shadow-md' :
          ...
          <div className="bg-gray-50 rounded-lg p-4 transition-all duration-300">
            {current === 0 && (
              <div className="space-y-3">
                ...
                {genMsg && <p className={`text-xs px-3 py-1.5 rounded ${genMsg.ok ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>{genMsg.text}</p>}
              </div>
            )}
            ...
            {current === 2 && (
              <div className="space-y-3">
                <p className="text-xs text-gray-500">
                  {adminStampedFile ? '授权代表（LIVANTO GmbH）已完成盖章回传，请下载存档。'
                    : '已收到您的签字盖章合同，授权代表方盖章完成后将在此显示，届时请下载。'}
                </p>
                {adminStampedFile ? (
                  <a href={`/api/uploads/${adminStampedFile.id}/download`}
                    className="flex items-center justify-between bg-blue-50 border border-blue-100 rounded-lg px-4 py-3 hover:bg-blue-100 transition-colors w-full" download>
            ...
            {current === 3 && allDone && (
              <div className="text-center py-4 space-y-2">
                ...
                <div className="flex gap-2 justify-center pt-2 flex-wrap">
                  {adminStampedFile && (
                    <a href={`/api/uploads/${adminStampedFile.id}/download`}
                      className="px-4 py-1.5 bg-white border border-green-200 rounded-md text-xs text-green-600 hover:bg-green-50 transition-colors" download>
                      🏛️ 下载盖章合同
                    </a>
                  )}
                  {mySignedUpload && (
                    <a href={`/api/uploads/${mySignedUpload.id}/download`}
                      className="px-4 py-1.5 bg-white border border-gray-200 rounded-md text-xs text-gray-500 hover:bg-gray-50 transition-colors" download>
                      📤 下载上传件
                    </a>
                  )}
                </div>
